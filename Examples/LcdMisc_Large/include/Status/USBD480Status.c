...
struct USBD480Status {
	var %font, %modeFont, %bpmFont, %freqFont, %counters, %vlcFont;
	//var %driveDelayShow;
	...
	function USBD480Status (){
		...
		%vlcFont = Font("Arial", 32,0,0,0,CLEARTYPE_QUALITY);
		%bpmFont = Font("Insane Hours",60,0,0,0,CLEARTYPE_QUALITY);
		%freqFont = Font("Insane Hours",44,0,0,0,CLEARTYPE_QUALITY);
		...
		//%tcxRoute = Tcx(/*"filename.tcx"*/, "C:/Users/Administrator/AppData/Roaming/Garmin/Devices/3825666043/History/");
		//%tcxRoute.update();
	}
	...
	function Draw ($event, $param, $name, $res) {
		...
		UseFont(%font);
		DisplayTextShadowed("CPU: "+s FormatValue(%counters.cpuData["_Total"], 0, 1)+s "%", 1, 34, RGBA(0x50,0x8D,0xC5,156), RGBA(255,255,255,240));
		DisplayTextShadowed("Mem: "+s FormatValue($memory[1]/1024.0/1024.0), 1, 56, RGBA(0x50,0x8D,0xC5,156), RGBA(255,255,255,240));
		...
		if (%hrmIsConnected){
			...
			UseFont(%freqFont);
			DisplayTextRightShadowed($maxClk, DWIDTH-8, 38, RGBA(0xFF,0x1D,0x0,255), RGBA(255,255,255,255));
			UseFont(%font);
		}
