...
#include "antique_clock.cpp"  // the clock, RGB565 format, no transparency
#include "long_hand.cpp"  // long hand, RGB32 format (with transparency)
#include "small_hand.cpp" // small hand, RGB32 format (with transparency)
...
void drawClock(float angle, float scale= 1.0f, float opacity = 1.0f)
    ...
    if (opacity == 1.0f)
        ...
        im.blitScaledRotated(antique_clock, { 234/2, 234/2 }, { SLX2,SLY2 }, scale, angle);  
        }
    else 
        {
        im.blitScaledRotated(antique_clock, { 234/2, 234/2 }, { SLX2,SLY2 }, scale, angle, opacity);  
        }
    } 
...
int main ()
{
    ...
    while ((t = em) < 1000){      
        ...
        drawClock(200 - t/5.0, sc);
        drawSmallHand(t/10.0, sc);
        drawLongHand(t/2.0, sc); 
