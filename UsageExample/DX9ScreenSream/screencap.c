...
int d3d9_loop (d3d9ctx_t *ctx, const int bestFit, const int followCursor)
{
	...
	while (!kbhit()){
		...
		if (IDirect3DDevice9_GetFrontBufferData(ctx->device, 0, ctx->surface) == S_OK){
			if (IDirect3DSurface9_LockRect(ctx->surface, &ctx->rc, NULL, D3DLOCK_DONOTWAIT|D3DLOCK_NO_DIRTY_UPDATE|D3DLOCK_READONLY) == S_OK){
	  		...
	  		libTeensyRawHid_WriteImage(&hid_ctx, ctx->out_buffer);
		}else{
			Sleep(20);
		}
		ct++;
...
int gdi_loop (gdictx_t *ctx, const int bestFit, const int followCursor, const int usePrintWinAPI)
{
...
int main (int margc, char **cargv)
{
	...
	if (!gdiCapture){
		...
		if (d3d9_init(&d3d_ctx, 0, &pt, hwnd)){
			d3d9_loop(&d3d_ctx, bestFit, followCursor);
			d3d9_close(&d3d_ctx);
		}
	}else{
		gdictx_t gdi_ctx;
		...
		if (gdi_init(&gdi_ctx, 0, &pt, hwnd)){
			gdi_loop(&gdi_ctx, bestFit, followCursor, usePrintWinAPI);
			gdi_close(&gdi_ctx);
		}		
	}
