// https://github.com/KurtE/GC9A01A_SPI
// http://forum.pjrc.com/threads/26305-Highly-optimized-GC9A01A-(320x240-TFT-color-display)-library

// This is the GC9A01A_t3n driver with the bloat removed + updates
...
#define COUNT_WORDS_WRITE       ((GC9A01A_TFTHEIGHT * GC9A01A_TFTWIDTH) /  SCREEN_DMA_NUM_SETTINGS) // Note I know the divide will give whole number

#if defined(__MK66FX1M0__)
// T3.6 use Scatter/gather with chain to do transfer
DMASetting GC9A01A_SPI::_dmasettings[3][3];
//DMAChannel GC9A01A_SPI::_dmatx;
#elif defined(__IMXRT1052__) || defined(__IMXRT1062__) // Teensy 4.x
// DMASetting 	GC9A01A_SPI::_dmasettings[4];
// DMAChannel 	GC9A01A_SPI::_dmatx;
#elif defined(__MK64FX512__)
// T3.5 - had issues scatter/gather so do just use channels/interrupts
// and update and continue
DMAChannel GC9A01A_SPI::_dmatx;
DMAChannel GC9A01A_SPI::_dmarx;
uint16_t GC9A01A_SPI::_dma_count_remaining;
uint16_t GC9A01A_SPI::_dma_write_size_words;
volatile short _dma_dummy_rx;
#endif

GC9A01A_SPI *GC9A01A_SPI::_dmaActiveDisplay[3] = {0, 0, 0};

#if defined(__IMXRT1052__) || defined(__IMXRT1062__) // Teensy 4.x
// On T4 Setup the buffers to be used one per SPI buss... 
// This way we make sure it is hopefully in uncached memory
GC9A01A_DMA_Data_t GC9A01A_SPI::_dma_data[3];   // one structure for each SPI buss... 
#endif

// volatile uint8_t  	GC9A01A_SPI::_dma_state = 0;  // Use pointer to this
// as a way to get back to object...
// volatile uint32_t	GC9A01A_SPI::_dma_frame_count = 0;	// Can return a
// frame count...

void GC9A01A_SPI::dmaInterrupt(void) {
  if (_dmaActiveDisplay[0]) {
    _dmaActiveDisplay[0]->process_dma_interrupt();
  }
}
void GC9A01A_SPI::dmaInterrupt1(void) {
  if (_dmaActiveDisplay[1]) {
    _dmaActiveDisplay[1]->process_dma_interrupt();
  }
}
void GC9A01A_SPI::dmaInterrupt2(void) {
...
void GC9A01A_SPI::process_dma_interrupt(void) {
#ifdef DEBUG_ASYNC_LEDS
...
//=======================================================================
void GC9A01A_SPI::setFrameBuffer(uint16_t *frame_buffer) {
#ifdef ENABLE_GC9A01A_FRAMEBUFFER
...
#ifdef ENABLE_GC9A01A_FRAMEBUFFER
void GC9A01A_SPI::setFrameCompleteCB(void (*pcb)(), bool fCallAlsoHalfDone) {
...
uint8_t GC9A01A_SPI::useFrameBuffer(
...
void GC9A01A_SPI::freeFrameBuffer(void) // explicit call to release the buffer
{
#ifdef ENABLE_GC9A01A_FRAMEBUFFER
...
void GC9A01A_SPI::updateScreen(void) // call to say update the screen now.
{
...
//==============================================
#ifdef ENABLE_GC9A01A_FRAMEBUFFER
void GC9A01A_SPI::initDMASettings(void) {
...
void GC9A01A_SPI::dumpDMASettings() {
#ifdef DEBUG_ASYNC_UPDATE
#if defined(__MK66FX1M0__)
  // T3.6
...
bool GC9A01A_SPI::updateScreenAsync(
...
void GC9A01A_SPI::endUpdateAsync() {
// make sure it is on
#ifdef ENABLE_GC9A01A_FRAMEBUFFER
...
void GC9A01A_SPI::waitUpdateAsyncComplete(void) {
#ifdef ENABLE_GC9A01A_FRAMEBUFFER
#ifdef DEBUG_ASYNC_LEDS
  // digitalWriteFast(DEBUG_PIN_3, HIGH);
#endif
...
void GC9A01A_SPI::setAddrWindow (uint16_t x0, uint16_t y0, uint16_t x1,
                                uint16_t y1) {
  beginSPITransaction(_SPI_CLOCK);
  setAddr(x0, y0, x1, y1);
...
void GC9A01A_SPI::setRotation(uint8_t m) {
...
void GC9A01A_SPI::invertDisplay (boolean i)
{
  beginSPITransaction(_SPI_CLOCK);
...
void GC9A01A_SPI::setFrameRateControl(uint8_t mode) {
  // Do simple version
...
void GC9A01A_SPI::writeRect (int16_t x, int16_t y, int16_t w, int16_t h, const uint16_t *pcolors)
{
...
void GC9A01A_SPI::fillRect (int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color)
{
    beginSPITransaction(_SPI_CLOCK);
    setAddr(x, y, x + w-1, y + h-1);
    writecommand_cont(GC9A01A_RAMWR);
...
void GC9A01A_SPI::fillScreen (uint16_t color)
{
#ifdef ENABLE_GC9A01A_FRAMEBUFFER
...
FLASHMEM void GC9A01A_SPI::begin (uint32_t spi_clock, uint32_t spi_clock_read)
{
  ...
  if (SPI.pinIsMOSI(_mosi) && SPI.pinIsSCK(_sclk)) {
    ...
    //Serial.println( "GC9A01A_SPI: The IO pins on the constructor are not valid SPI pins");
 ...
 // Teensy 4.x
  ...
  if (_pspi->pinIsChipSelect(_dc)) {
    ...
    // Serial.println("GC9A01A_SPI: DC is not valid hardware CS pin");
...
uint8_t GC9A01A_SPI::getRotation (void)
{
	return rotation;
}

void GC9A01A_SPI::sleep (bool enable)
{
  beginSPITransaction(_SPI_CLOCK);
  if (enable) {
...
void GC9A01A_SPI::sendCommand (uint8_t commandByte, const uint8_t *dataBytes, uint8_t numDataBytes)
{
    beginSPITransaction(_SPI_CLOCK);
...
// From Spin:
#if defined(KINETISK)
void GC9A01A_SPI::waitFifoNotFull(void) {
  uint32_t sr;
...
void GC9A01A_SPI::waitFifoEmpty(void) {
  uint32_t sr;
  uint32_t tmp __attribute__((unused));
  do {
    sr = _pkinetisk_spi->SR;
...
uint16_t GC9A01A_SPI::waitTransmitCompleteReturnLast() {
  uint32_t val=0;
  //    // digitalWriteFast(2, HIGH);
